...
* MAIN PROGRAM VARIABLES
      ...
      real*8 step        ! Integration step size (secs).  We initially
                         ...
                         ! reset as needed.
      complex*16 Pos, Vel  ! Position and velocity of bike 
...
****  Run test runs to see what integration step size we need.
      step = 2.0d0 ! Initially set step size to 1-sec (tested in int_soln)
...
*     Assume a speed of 10 m/s convert distance into time difference
     ...
     .           step .gt.1.d-3 )
          ...
          call Int2D(step, Trav_time1, Pos, Vel )
          step = step/2
          Pos = dcmplx(0.,Bs)
          Vel = dcmplx(0.,0.)
          Trav_time2 = 0.d0
          call Int2D(step, Trav_time2, Pos, Vel )
...
CTITLE INT2d
      ...
      subroutine Int2d(step,int_time, Pos, Vel )
      implicit none
...
* PASSED VARIABLES
      ...
      real*8 int_time    ! Integration time.  If this is zero initially
                         ...
                         ! on time.
      complex*16 Pos, Vel   ! Complex representation of 2-components of
                         ! position and velocity

* LOCAL VARIABLES
      complex*16 P, V     ! Intermediate position and velocities needed
      real*8 TE           ! Intermediate energy
      complex*16 k1, k2, k3, k4   ! Coefficients needed for Runge Kutta integration

      complex*16 accel    ! Complex function that return x acceleration in real part
...
***** Initialize position
      ...
      if( outT ) then
         write(*,110) 
 110     format('O*    Time          X_pos           Z_pos',
     .          '           X_vel          Z_vel     Energy',/,
     .          'O*    (sec)          (m)             (m) ',
     .          '           (m/s)          (m/s)    (Joules)') 
         write(*,160) 'O', time, Pos, Vel, tot_energy
      end if
...
****     Compute the accelerations we need
         k1 = ustep*accel(time, Pos, Vel, AP1 )

         P = Pos + ustep*Vel/2.d0 +ustep*k1/8.d0
         V = Vel + k1/2.d0
         k2 = ustep*accel(time+ustep/2.d0, P, V, AP2)

         P = Pos + ustep*V/2.d0 + ustep*k1/8.d0
         V = Vel + k2/2.d0 
         k3 = ustep*accel(time+ustep/2.d0, P, V, AP2 )

         P = Pos + ustep*Vel + ustep*k3/2.d0
         V = Vel + k3
         k4 = ustep*accel(time+ustep, P, V, AP3)
...
*        (Also called Sumpson's rule)
         TE = tot_energy + (ustep/2)*(AP1+4*AP2+AP3)/3.d0

****     See if we have passed the end of the track.  If we have then
*        take smaller steps.
         if( dble(P)-Track_len.gt.0 ) then
             tpast = (dble(P)-Track_len)/dble(Vel) 
             ustep = abs(tpast/2.d0)
             if( abs(dble(P)-Track_len).lt. terr/10 ) then
                 time = T
                 Pos = P
                 Vel = V
                 tot_energy = TE
...
*            Continue with integration
             ...
             tot_energy = TE
             if( outT ) then 
                if( abs(T-nint(T/out_int)*out_int).le.1.d-6 ) then
                   write(*,160) 'O',time, Pos, Vel, tot_energy
!                else
!                   write(*,160) 'T',time, Pos, Vel, tot_energy
                endif
 160            format(a,1x,F10.3, 2F15.4, 2F15.4,1x, F12.2)
             end if
         end if
...
*     horizontal distance for this effect.
      tpast = dble(Pos)/dble(Vel)
c      Dist = realpart(Pos) - realpart(Vel)*tpast
      Dist = dble(Pos) - dble(Vel)*tpast
      int_time = time
...
CTITLE ACCEL
      ...
      complex*16 function accel( time, P, V, AP)
      implicit none
...
* PASSED VARIABLES
      ...
      complex*16 P, V     ! X (horizontal) and Z (vertical) position and
                         ! velocity of object (m, m/s) in complex form
                         ! so that vector calculations can be done.
      real*8 AP          ! Actual power supplied by rider 

* LOCAL VARIABLES
      complex*16 gacc, dacc, racc  ! Gravity, Drag and rolling accelerations
      complex*16 sacc, cacc  ! Surface normal force and centripical force 
                             ! convert to acceleration
      complex*16 facc        ! Rider force acceleration
      complex*16 ldir    ! Direction perpendicular to motion direction
      complex*16 Usurf   ! Unit vector along road surface
      real*8 Vmag        ! Magnitude of velocity
      real*8 Rho_h       ! Density of air at our height
      real*8 theta       ! Surface slope
      real*8 IRc, dy2dx2 ! Curvature of surface and second derivative
      real*8 F_mag       ! Magnitude of rider force (N)


***** Compute the accelations:
*     GRAVITY: Depends of Height
      gacc = dcmplx(0.d0, -g_0)

      theta = atan(Slope + 
     .             As*cos(2*pi*dble(P)/Lambda)*2*pi/Lambda - 
     .             Bs*sin(2*pi*dble(P)/Lambda)*2*pi/Lambda)

      Usurf = dcmplx(cos(theta),sin(theta))

*     DRAG: Acts opposite to the motion vector (since wind is fixed
*     relative to motion direction)
      Vmag = abs(V)
      Rho_h = rho_air
      dacc = -(Cd*Rho_h*Vmag*V/2)/Mass*Area  ! Note: This is Vmag^2*V_unit_vector
      racc = -g_0*Cr*Usurf

*     Get the acceleration perpendicular to surface
      sacc = dcmplx(-g_0*cos(theta)*sin(theta), g_0*cos(theta)**2)

*     get Centripical acceleration
      dy2dx2 =  -As*sin(2*pi*dble(P)/Lambda)*(2*pi/Lambda)**2 - 
     .           Bs*cos(2*pi*dble(P)/Lambda)*(2*pi/Lambda)**2
*     Use the second derivative to compute curvature and centripetal 
*     acceleration.  (Factor after ! is to get curvature but this seems
*     to generate greater deviation from path).
      IRc = (dy2dx2)    ! sqrt( (1+tan(theta)**2)**3 )
      cacc = vmag**2*dcmplx(sin(theta),cos(theta))*IRc
...
*     Get the rider power
      ...
      facc = F_mag*Usurf/mass
      AP = F_mag*Vmag


****  Add the acceleration together
      accel = gacc + dacc + racc + sacc + cacc + facc 
